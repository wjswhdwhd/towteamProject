...
<script type="text/javascript">
	$(function() {
		$("#sample6_address2").keyup(
				function() {
					var address = $("#sample6_address").val()
							+ $("#sample6_address2").val();
					$("#m_address").val(address);
					$("#m_zipcode").val($("#sample6_postcode").val());
				});
		$("#addressModifiBtn").on("click", function() {
			$.ajax({
				url:"/member/addressmodifi",
				type:"post",
				data:{"m_address":$("#m_address").val(),
...
<body>
	...
	<div>
		<input class="form-control" type="text" id="sample6_postcode"
			placeholder="우편번호" required="required" readonly="readonly"> <input
		...
		<input class="form-control" type="text" id="sample6_address2"
			placeholder="상세주소"><input type="hidden" name="m_zipcode"
			id="m_zipcode"><input type="hidden" name="m_address"
			id="m_address">
	</div>
	<div>
		<button id="addressModifiBtn">주소 변경하기</button>
	</div>
</body>
</html>